#!/usr/bin/env python3
"""
Cisco Catalyst Center – Italy Site Törlő
Törli az "Italy" nevű site-ot és az összes alatta lévő gyermek site-ot.
...
# ─────────────────────────────────────────────
#  Italy és gyermekeinek összegyűjtése
# ─────────────────────────────────────────────
...
def collect_subtree(sites: dict, root_id: str) -> list[str]:
    ...
    result.append(root_id)   # a szülőt mindig a gyermekei után adjuk hozzá
    return result
...
def main():
    print("\n" + "="*60)
    print("  CISCO CATALYST CENTER – ITALY SITE TÖRLŐ")
    print("="*60 + "\n")
    ...
    try:
        ...
        # Italy megkeresése
        italy_id = find_site_by_name(sites, TARGET_SITE)
        if not italy_id:
            print(f"[!] Nem található '{TARGET_SITE}' nevű site. Nincs mit törölni.")
            return

        print(f"[✓] '{TARGET_SITE}' site azonosítója: {italy_id}")

        # Összes érintett site összegyűjtése (leaf-first sorrend)
        to_delete = collect_subtree(sites, italy_id)
